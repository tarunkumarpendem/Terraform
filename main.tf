...
resource "aws_launch_template" "lb_launch_template" {
  image_id = var.instances_details.ami_id
  instance_type = var.instances_details.instance_type
  key_name = var.instances_details.key_pair
  name = var.instances_details.launch_template_name
  ...
  tag_specifications {
    resource_type = var.instances_details.launch_template_resource_type
    tags = {
      Name = var.instances_details.launch_template_tag
    }
  }
  network_interfaces {
    associate_public_ip_address = true
    subnet_id = aws_subnet.lb_subnets[0].id
...
resource "aws_autoscaling_group" "lb-asg" {
  name = var.instances_details.auto_scaling_group_name
  max_size = var.instances_details.asg_max_size
  min_size = var.instances_details.asg_min_size
  desired_capacity = var.instances_details.asg_desired_capacity
  availability_zones = [ var.network_details.availability_zones[0] ]
  health_check_grace_period = "300"
  health_check_type = "ELB"
  launch_template {
    id = aws_launch_template.lb_launch_template.id
    version = "$Latest"
  }
  tag {
    key = "Name"
    value = var.instances_details.auto_scaling_group_name
...
resource "aws_autoscaling_attachment" "lb_asg_attachment" {
  autoscaling_group_name = aws_autoscaling_group.lb-asg.name
